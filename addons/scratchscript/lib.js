...
class Lexer2 {
    ...
    expression() {
        ...
        // this should catch the whole number as there are no context terminating chars in a number
            ...
            }, TreeNode.TYPES.LITERAL, [], this.expressionStartIndex, this.currentToken.endChar);
            this.inc();
            return node;
        }
        else if (this.currentToken.value === '\n') {
            ...
            var node = new TreeNode({
                value: this.currentToken.value
            }, TreeNode.TYPES.NAME, [], this.expressionStartIndex, this.currentToken.endChar);
            this.inc();
    ...
    string(strType) {
        ...
        var slashDepth = 0;
        this.inc();
        while (this.currentToken?.value != strType && this.tokenIndex < this.tokens
            .length /* && this.slashDepth % 2 != 1 */) {
    ...
    comment() {
        ...
        return new TreeNode({
            value: commentValue
        }, TreeNode.TYPES.COMMENT, [], this.expressionStartIndex, this.currentToken?.endChar);
    }
...
class ErrorHandler extends EventTarget {
    ...
    unmatchedBlock(blockSet, parentBlock, parentOpcode) {
        ...
        const correspondingText = this.text.slice(
            blockSet[0]?.startChar,
            blockSet[blockSet.length - 1]?.endChar,
        );
        const error = new ParseError(
            `"${correspondingText}" did not find a match between ${blockSet[0]?.startChar
            } and ${blockSet[blockSet.length - 1]?.endChar}`,
        );
        throw error;
    }
}
            ...
            switch (type) {
                case BlockIOType.STRING:
                    return [
                        ...
                        TreeNode.TYPES.BOOL_REPORTER,
                        TreeNode.TYPES.ROUND_REPORTER,
                    ];
...
class TokenMatch {
    ...
    check(token, ruleSet) {
        if (this.types.length != 0 && !this.types.includes(token?.type))
            return false;
        // only check values for 'name' tokens
        if (
            token?.type === TreeNode.TYPES.NAME &&
            this.values.length != 0 &&
            !this.values.includes(token?.value)
        )
            return false;
        if (
            token?.type === TreeNode.TYPES.FIELD &&
            typeof this.values === "string" &&
            !ruleSet.blockCategory
                .fieldByName(this.values)
                .keys()
...
class Rule {
    ...
    input(type, name, values = []) {
        this.tokenMatches.push(
            new TokenMatch(values, BlockIOType.toTypes(type), name),
        );
        return this;
    }
    ...
    _checkTokenList(blockSet, ruleSet, tokenList = []) {
        ...
        for (let tokenI = 0; tokenI < blockSet.length; tokenI++) {
            ...
            if (
                tokenI >= tokenList.length &&
                blockSet[tokenI + commentCount]?.type != TreeNode.TYPES.COMMENT
            ) {
    ...
    getAlias(blockSet, parentContext, ruleSet) {
        ...
        if (
            this.parentContextRule &&
            !this.parentContextRule.check(parentContext, ruleSet)
        )
            return false;
    ...
    parseInput(token, ruleSet, id, inputRule) {
        ...
        if (
            token?.type === TreeNode.TYPES.FIELD &&
            inputRule.types.includes(BlockIOType.FIELD)
        ) {
            ...
            input = ruleSet.matchNode(
                ...
                new ParentContext(
                    token,
                    this.opcode,
                    BlockIOType.toType(inputRule.types),
                ),
            );
            input.parent = id;
        } else if (
            token?.type === TreeNode.TYPES.LITERAL &&
            [BlockIOType.STRING, BlockIOType.NUMBER, BlockIOType.ANGLE].some((t) =>
                inputRule?.types.includes(t),
            )
        ) {
            ...
            [TreeNode.TYPES.BOOL_REPORTER, TreeNode.TYPES.ROUND_REPORTER].includes(
                token.type,
            )
        ) {
            input = ruleSet.matchNode(
            ...
            input =
        /*ruleSet.match([token], parentBlock, parentOpcode) || */ token.value;
            if (
                input.id /* ie it is an instance of RuleMatch and not a string value */
            )
                input.parent = id;
        }
        return input;
    ...
    match(blockSet, ruleSet, parentContext) {
        ...
        for (
            let tokenI = 0;
            tokenI < alias.length ||
            blockSet[tokenI + comments.length]?.type === TreeNode.TYPES.COMMENT;
            tokenI++
        ) {
...
class RuleSet {
        ...
        return blockNode.children.map((b) =>
            this.matchNode(
                ...
                new ParentContext(
                    b,
                    parentOpcode,
                    parentInput || BlockIOType.STATEMENT,
                ),
            ),
        );
    }
    ...
    matchNode(node, parentContext, _blockCategory) {
        return this.findMatch(
            node.children,
            parentContext || new ParentContext(node),
            _blockCategory,
        );
    }
...
class Block extends Rule {
        ...
        try {
            var node = new TreeNode({}, TreeNode.TYPES.ROUND_REPORTER, [
                new TreeNode(
                    ...
                    this.extraInputData[input.inputName].fieldType
                        ? TreeNode.TYPES.FIELD
                        : TreeNode.TYPES.LITERAL,
                ),
            ]);
            ...
            match = _blockCategory.ruleSet.matchNode(
                ...
                new ParentContext(
                    node,
                    ruleMatch.opcode,
                    BlockIOType.toType(input.types),
                ),
        ...
        for (let i = 0; i < rawInputs.length; i++) {
            ...
            if (Array.isArray(inputMatch)) {
                ...
                if (input.types.includes(BlockIOType.RAW_FIELD)) {
                    console.log(this.extraInputData[input.inputName], _blockCategory
                        .fieldByName(this.extraInputData[input.inputName].fieldType));
                    blockData.fields[input.inputName] = Object.assign(
                        { name: input.inputName },
                        _blockCategory
                            .fieldByName(this.extraInputData[input.inputName].fieldType)
                            .toObjectFromName(inputMatch),
                    );
                } else {
    ...
    decompile(id, data, _blockCategory) {
        ...
        for (let tokenI = 0; tokenI < this.tokenMatches.length; tokenI++) {
            ...
            let inputData = isInput
                ? thisData.inputs[token.inputName]
                : thisData.fields[token.inputName];
                ...
                switch (type) {
                    ...
                    case BlockIOType.RAW_FIELD:
                        decompileString += `[${escapeQuotationMarks(
                            this.extraInputData[token.inputName].fieldType.geByValue(
                                inputData.value,
                            ),
                        )}]`;
                        break;
                    ...
                    default:
                        console.error(
                            "Expected field type " + type + " on " + token.inputName,
                        );
                }
            } else {
        ...
        if (
            this.parentContextRule.blockMatch.types.includes(
                TreeNode.TYPES.BOOL_REPORTER,
            )
        ) {
            ...
            this.parentContextRule.blockMatch.types.includes(TreeNode.TYPES.STATEMENT)
        ) {
            decompileString = decompileString + "\n";
        }
        if (thisData.next) {
...
class BlockCategory {
    ...
    merge(other) {
        return new BlockCategory(
            this.blocks.concat(other.blocks),
            this.fields.concat(other.fields),
        );
    }
...
class CoreCategory extends BlockCategory {
    ...
    init() {
        ...
        this.comment.tokenMatches.push(
            new TokenMatch([], [TreeNode.TYPES.COMMENT]),
        );
...
class MotionCategory extends BlockCategory {
    ...
    init() {
        ...
        this.pointtowards
            .name("point")
            .name("towards")
            .field("TOWARDS", this.point_field, "mouse-pointer");
        ...
        this.goto_menu
            .shadow()
            .parentOpcode("motion_goto")
            .raw_field("TO", this.target_field)
            .output_field();
        ...
        this.goto
            ...
            .name("go")
            .name("to")
            .field("TO", this.target_field, "mouse-pointer");
...
class LooksCategory extends BlockCategory {
    ...
    init() {
        ...
        this.costume
            .raw_field("COSTUME", this.costume_field)
            .reporter()
            .output_field()
            .shadow();
        ...
        this.switchcostumeto
            .name("switch")
            .name("costume")
            .name("to")
            .field("COSTUME", this.costume_field, "costume1");
        ...
        this.costumenumbername
            .name("costume")
            .raw_field("NUMBER_NAME", this.costumenumbername_field)
            .reporter();
        this.block(this.costumenumbername);

        this.backdrop = new BlockShorthand("looks_backdrop");
        this.backdrop
            .raw_field("BACKDROP", this.backdrop_field)
            .output_field()
            .reporter()
            .shadow();
        ...
        this.switchbackdroptoandwait = new BlockShorthand(
        ...
        this.backdropnumbername
            .name("backdrop")
            .raw_field("NUMBER_NAME", this.backdropnumbername_field)
            .reporter();
        ...
        this.gotofrontback
            .name("go")
            .name("to")
            .raw_field("FRONT_BACK", this.frontback_field)
            .name("layer");
        this.block(this.gotofrontback);

        this.goforwardbackwardlayers = new BlockShorthand(
...
class SoundCategory extends BlockCategory {
    ...
    init() {
        ...
        this.sounds_menu
            .raw_field("SOUND_MENU", this.sounds_menu_field)
            .output_field()
            .reporter()
            .shadow();
        ...
        this.seteffectto
            .name("set")
            .raw_field("EFFECT", this.soundeffect_field)
            .name("to")
            .number("VALUE", "100");
        this.block(this.seteffectto);
...
class EventCategory extends BlockCategory {
    ...
    init() {
        ...
        this.whenthisspriteclicked = new BlockShorthand(
        ...
        this.whenbroadcastrecieved = new BlockShorthand(
        ...
        this.whenbackdropswitchesto = new BlockShorthand(
...
class BlockScope {
    ...
    soundsMenu() {
        ...
        if (this.target && this.target.sprite.sounds.length > 0) {
            menu = this.target.sprite.sounds.map((sound) => [
                sound.name,
                sound.name,
            ]);
        }
    ...
    backdropsMenu() {
        if (
            this.project.vm.runtime.targets[0] &&
            this.project.vm.runtime.targets[0].getCostumes().length > 0
        ) {
        ...
        for (const targetId in this.project.vm.runtime.targets) {
            ...
            if (this.project.vm.runtime.targets[targetId].isOriginal) {
                if (!this.project.vm.runtime.targets[targetId].isStage) {
                    if (
                        this.project.vm.runtime.targets[targetId] ===
                        this.project.vm.editingTarget
                    ) {
                        continue;
                    }
                    sprites.push([
    ...
    sensing_of(ofTarget) {
        ...
        if (this.target) {
            ...
            const stageVariableOptions = this.project.vm.runtime
                .getTargetForStage()
                ...
                return stageOptions.concat(
                    spriteOptions.concat(
                        this.project.vm.runtime.targets
                            ...
                            .map((t) =>
                                t.getAllVariableNamesInScopeByType("", true),
                            )
                            .flat(),
                    ),
                );
    ...
    addToTarget(constructive = false) {
        for (let varI; varI < this.newVariables.length; varI++)
            this.newVariables[varI].attach(this.target);
        this.target.blocks._blocks = constructive
            ? Object.fromEntries(
                Object.entries(this.target.blocks._blocks).concat(
                    Object.entries(this.blocks),
                ),
            )
            : this.blocks;
        this.target.blocks._scripts = constructive
            ? this.target.blocks._scripts.concat(this.scripts)
            : this.scripts;
