...
class TargetVMSnapshot {
  constructor(target) {
    ...
    this.variables = Object.values(target.variables).map((v) => ({
      id: v.id,
      name: v.name,
      type: v.type,
      isCloud: v.isCloud,
...
class ThreadVMSnapshot {
  ...
  restore(vm) {
    var thread = vm.runtime._pushThread(this.topBlock, vm.runtime.getTargetById(this.target), {
...
class VMSnapshot {
  constructor(vm) {
    if (vm) {
      // then we're deserialising so we keep them empty
